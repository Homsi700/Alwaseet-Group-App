...
export default function NewProductPage() {
  ...
  // في الواقع، سنستخدم useCreateProduct hook
  ...
  const createProductMutation = {
    mutateAsync: async (data: any) => {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({ id: 'new-product-id', ...data });
        }, 1500);
      });
    },
  };
  ...
  const handleCancel = () => {
    if (hasChanges) {
      if (confirm('هل أنت متأكد من رغبتك في إلغاء إنشاء المنتج؟ سيتم فقدان جميع البيانات المدخلة.')) {
        router.push('/inventory');
      }
    } else {
      router.push('/inventory');
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold text-foreground">إضافة منتج جديد</h1>
          </div>
          <p className="text-muted-foreground mt-1">إنشاء منتج جديد وإضافته إلى المخزون</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          >
            <Save className="ml-2 rtl:mr-2 h-5 w-5 icon-directional" /> حفظ المنتج
            {isSaving && (
            ...
            <CardContent className="space-y-4">
              {productData.name ? (
                <div className="space-y-2">
                  ...
                  {productData.sku && (
                    <div className="flex items-center text-sm text-muted-foreground">
                      <Barcode className="h-3.5 w-3.5 ml-1 rtl:mr-1 icon-directional" />
                      <span>{productData.sku}</span>
                    </div>
                  )}
                  {productData.categoryId && (
                    <div className="text-sm text-muted-foreground">
                      الفئة: {mockCategories.find(c => c.id === productData.categoryId)?.name}
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center py-6 text-muted-foreground">
                  ...
                  <p>لم يتم إدخال معلومات المنتج بعد</p>
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">له متغيرات:</span>
                  <span className="font-medium">{productData.hasVariants ? 'نعم' : 'لا'}</span>
                </div>
              </div>
            </CardContent>
            <CardFooter className="flex flex-col gap-2">
              <Button className="w-full" onClick={handleSaveProduct} disabled={isSaving || !hasChanges}>
                <Save className="ml-2 rtl:mr-2 h-4 w-4 icon-directional" /> حفظ المنتج
                {isSaving && (
                  <Loader2 className="ml-2 rtl:mr-2 h-4 w-4 animate-spin" />
                )}
              </Button>
