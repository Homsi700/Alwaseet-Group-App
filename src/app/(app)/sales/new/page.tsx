...
export default function NewSalePage() {
  ...
  const handleSaveSale = async () => {
    ...
    try {
      // في الواقع، سنستخدم createSaleMutation.mutateAsync
  ...
  const handleCancel = () => {
    if (saleData.items && saleData.items.length > 0) {
      if (confirm('هل أنت متأكد من رغبتك في إلغاء الفاتورة؟ سيتم فقدان جميع البيانات المدخلة.')) {
        router.push('/sales');
      }
    } else {
      router.push('/sales');
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold text-foreground">إضافة فاتورة مبيعات جديدة</h1>
          </div>
          <p className="text-muted-foreground mt-1">إنشاء فاتورة مبيعات جديدة وإضافة المنتجات إليها</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <Button 
            ...
            disabled={isSaving}
          >
            <Save className="ml-2 rtl:mr-2 h-5 w-5 icon-directional" /> حفظ الفاتورة
            {isSaving && (
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardFooter className="flex flex-col gap-2">
              <Button className="w-full" onClick={handleSaveSale} disabled={isSaving}>
                <Save className="ml-2 rtl:mr-2 h-4 w-4 icon-directional" /> حفظ الفاتورة
                {isSaving && (
                  <Loader2 className="ml-2 rtl:mr-2 h-4 w-4 animate-spin" />
                )}
              </Button>
              <Button variant="outline" className="w-full">
                <Printer className="ml-2 rtl:mr-2 h-4 w-4 icon-directional" /> معاينة الطباعة
              </Button>
            </CardFooter>
          </Card>
...
function Banknote(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      {...props}
      xmlns="http://www.w3.org/2000/svg"
